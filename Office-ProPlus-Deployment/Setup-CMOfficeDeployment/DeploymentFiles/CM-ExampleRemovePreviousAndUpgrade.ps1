...
#from which the script is run. Any Office products 
...
if ($Office2016C2RExists) {
  Write-Host "Office 2016 Click-To-Run is already installed"
    ...
    if (!(Test-Path -Path $targetFilePath)) {     
       Generate-ODTConfigurationXml -Languages AllInUseLanguages -TargetFilePath $targetFilePath | Set-ODTAdd -Version $NULL -SourcePath $SourcePath -Channel $Channel -Bitness $Bitness | Set-ODTUpdates -Channel $Channel -UpdatePath $UpdateURLPath | Set-ODTDisplay -Level None -AcceptEULA $true | Out-Null
       ...
       if ($UseVolumeLicensing) {
        ...
        Set-ODTAdd -SourcePath $SourcePath -TargetFilePath $TargetFilePath -Channel $Channel -Bitness $Bitness | Set-ODTUpdates -Channel $Channel -UpdatePath $UpdateURLPath | Out-Null
    }
